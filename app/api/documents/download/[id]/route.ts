...
type PDFPage = ReturnType<Awaited<ReturnType<typeof PDFDocument.create>>["addPage"]>;
type PDFFont = Awaited<ReturnType<Awaited<ReturnType<typeof PDFDocument.create>>["embedFont"]>>;
...
// ── Shared helper: AeroTrack footer ──
...
// Reuses the same layout as /api/documents/render-8130-pdf
// ══════════════════════════════════════════════════════════════
async function render8130Pdf(content: Record<string, string>, hash?: string | null): Promise<Uint8Array> {
  ...
  // Form header
  page.drawRectangle({ x: MARGIN, y: y - 30, width: CONTENT_WIDTH, height: 35, color: LIGHT_GRAY });
  page.drawText("AUTHORIZED RELEASE CERTIFICATE", { x: MARGIN + 10, y: y - 18, size: 14, font: fontBold, color: DARK_BLUE });
  page.drawText("FAA Form 8130-3", { x: MARGIN + 10, y: y - 28, size: 7, font, color: GRAY });
  page.drawText("OMB No. 2120-0020", { x: PAGE_WIDTH - MARGIN - 100, y: y - 18, size: 7, font, color: GRAY });
  y -= 45;
  drawLine(page, y);

  // Block 1 & 2
  y -= 5;
  drawLabel(page, "1. Approving Civil Aviation Authority / Country", MARGIN + 5, y, font);
  y -= 12;
  page.drawText(content.block1 || "FAA", { x: MARGIN + 5, y, size: 10, font: fontBold, color: BLACK });
  drawLabel(page, "2. Authorized Release Document", midX + 10, y + 12, font);
  page.drawText(content.block2 || "Authorized Release Certificate", { x: midX + 10, y, size: 9, font, color: BLACK });
  page.drawLine({ start: { x: midX, y: y + 18 }, end: { x: midX, y: y - 16 }, thickness: 0.5, color: GRAY });
  y -= 25;
  drawLine(page, y);
  ...
  y -= 12;
  page.drawText(content.block3 || "—", { x: MARGIN + 5, y, size: 10, font: fontMono, color: DARK_BLUE });
  ...
  for (const line of block4Lines) {
    page.drawText(line.trim(), { x: midX + 10, y: block4Y, size: 8, font, color: BLACK });
    block4Y -= 11;
  }
  page.drawLine({ start: { x: midX, y: y + 18 }, end: { x: midX, y: Math.min(block4Y + 5, y - 10) }, thickness: 0.5, color: GRAY });
  y = Math.min(block4Y + 5, y - 15) - 10;
  drawLine(page, y);

  // Block 5
  y -= 5;
  drawLabel(page, "5. Work Order / Contract / Invoice Number", MARGIN + 5, y, font);
  y -= 12;
  page.drawText(content.block5 || "—", { x: MARGIN + 5, y, size: 9, font: fontMono, color: BLACK });
  y -= 15;
  drawLine(page, y);
  ...
  y -= 12;
  page.drawText(content.block6a || "—", { x: MARGIN + 5, y, size: 9, font: fontBold, color: BLACK });
  const row6Y = y - 20;
  drawLabel(page, "6b. Part Number", MARGIN + 5, row6Y + 8, font);
  page.drawText(content.block6b || "—", { x: MARGIN + 5, y: row6Y - 4, size: 10, font: fontMono, color: BLACK });
  drawLabel(page, "6c. Serial Number", MARGIN + col6Width * 2, row6Y + 8, font);
  page.drawText(content.block6c || "—", { x: MARGIN + col6Width * 2, y: row6Y - 4, size: 10, font: fontMono, color: BLACK });
  drawLabel(page, "6d. Qty", MARGIN + col6Width * 4, row6Y + 8, font);
  page.drawText(content.block6d || "1", { x: MARGIN + col6Width * 4, y: row6Y - 4, size: 10, font: fontMono, color: BLACK });
  y = row6Y - 20;
  ...
  for (const status of statuses) {
    const checked = (content.block6e || "").toLowerCase() === status.toLowerCase();
    page.drawRectangle({ x: statusX, y: y - 2, width: 8, height: 8, borderColor: BLACK, borderWidth: 0.5, color: checked ? rgb(0.15, 0.15, 0.15) : rgb(1, 1, 1) });
    if (checked) page.drawText("X", { x: statusX + 1.5, y: y - 0.5, size: 7, font: fontBold, color: rgb(1, 1, 1) });
    page.drawText(status, { x: statusX + 11, y, size: 7.5, font, color: BLACK });
  ...
  for (const rawLine of block7Lines) {
    ...
    const isHeading = trimmed === trimmed.toUpperCase() && trimmed.length < 50 && trimmed.length > 3;
    if (y < 80) {
      page.drawText(`Page ${pdf.getPageCount()}`, { x: PAGE_WIDTH - MARGIN - 40, y: 25, size: 7, font, color: GRAY });
      page = pdf.addPage([PAGE_WIDTH, PAGE_HEIGHT]);
      y = PAGE_HEIGHT - 50;
      drawLabel(page, "7. Description — Status / Work (continued)", MARGIN + 5, y, font);
      y -= 14;
    }
    y = drawWrappedText(page, trimmed, MARGIN + 10, y, CONTENT_WIDTH - 20, isHeading ? fontBold : font, isHeading ? 8.5 : 8);
    y -= 2;
  }
  y -= 5;
  drawLine(page, y);
  ...
  y -= 12;
  page.drawText("[X] Condition for safe operation    [X] FAR § 43.9, 14 CFR Part 145", { x: MARGIN + 10, y, size: 7.5, font, color: BLACK });
  y -= 15;
  drawLine(page, y);

  // Blocks 11 & 12
  y -= 5;
  drawLabel(page, "11. Approval / Authorization Number", MARGIN + 5, y, font);
  drawLabel(page, "12. Date", midX + 10, y, font);
  y -= 12;
  page.drawText(content.block11 || "—", { x: MARGIN + 5, y, size: 9, font: fontMono, color: BLACK });
  page.drawText(content.block12 || "—", { x: midX + 10, y, size: 9, font: fontMono, color: BLACK });
  y -= 20;
  drawLine(page, y);
  ...
  y -= 15;
  page.drawRectangle({ x: MARGIN + 10, y: y - 25, width: 180, height: 30, borderColor: GRAY, borderWidth: 0.5 });
  page.drawText(content.block13 || "[PENDING E-SIGNATURE]", { x: MARGIN + 20, y: y - 15, size: 8, font, color: GRAY });
  page.drawText(`Date: ${content.block12 || "—"}`, { x: MARGIN + 210, y: y - 5, size: 8, font, color: BLACK });
  page.drawText(`Auth: ${content.block11 || "—"}`, { x: MARGIN + 210, y: y - 18, size: 8, font, color: BLACK });
  y -= 40;
  drawLine(page, y);
  ...
  y -= 12;
  drawWrappedText(page, content.block14 || "—", MARGIN + 5, y, CONTENT_WIDTH - 10, font, 7.5);
...
// Clean structured document with labeled sections.
// ══════════════════════════════════════════════════════════════
async function render337Pdf(content: Record<string, unknown>, hash?: string | null): Promise<Uint8Array> {
  const pdf = await PDFDocument.create();
  const font = await pdf.embedFont(StandardFonts.Helvetica);
  const fontBold = await pdf.embedFont(StandardFonts.HelveticaBold);
  const fontMono = await pdf.embedFont(StandardFonts.Courier);
  ...
  // Header
  page.drawRectangle({ x: MARGIN, y: y - 30, width: CONTENT_WIDTH, height: 35, color: LIGHT_GRAY });
  page.drawText("MAJOR REPAIR AND ALTERATION", { x: MARGIN + 10, y: y - 18, size: 14, font: fontBold, color: DARK_BLUE });
  page.drawText("FAA Form 337 (10/06)", { x: MARGIN + 10, y: y - 28, size: 7, font, color: GRAY });
  page.drawText("OMB No. 2120-0020", { x: PAGE_WIDTH - MARGIN - 100, y: y - 18, size: 7, font, color: GRAY });
  y -= 50;
  drawLine(page, y);
  ...
  for (let i = 0; i < fields1.length; i++) {
    ...
    page.drawText(fields1[i].value || "—", { x, y: y - 11, size: 10, font: i === 0 ? fontMono : font, color: i === 0 ? DARK_BLUE : BLACK });
  }
  y -= 30;
  drawLine(page, y);
  ...
  page.drawText(owner.name || "—", { x: MARGIN + 5, y: y - 11, size: 9, font: fontBold, color: BLACK });
  const midX = MARGIN + CONTENT_WIDTH / 2;
  drawLabel(page, "Address", midX, y, font);
  const ownerAddr = (owner.address || "—").split("\n");
  let addrY = y - 11;
  for (const line of ownerAddr) {
    page.drawText(line.trim(), { x: midX, y: addrY, size: 8, font, color: BLACK });
    addrY -= 11;
  }
  ...
  y = drawSectionHeader(page, "3. TYPE AND UNIT IDENTIFICATION", y, fontBold);
  y -= 5;
  // Type checkbox
  drawLabel(page, "Type", MARGIN + 5, y, font);
  page.drawText(`[X] ${(content.repairType as string) || "Repair"}`, { x: MARGIN + 5, y: y - 11, size: 8, font, color: BLACK });
  drawLabel(page, "Unit", MARGIN + 100, y, font);
  page.drawText((content.unit as string) || "APPLIANCE", { x: MARGIN + 100, y: y - 11, size: 8, font: fontBold, color: BLACK });
  y -= 30;
  ...
  y -= 5;
  y = drawField(page, "A. Agency", conformity.agency?.split("\n")[0] || "—", MARGIN + 5, y, font, fontBold);
  y = drawField(page, "B. Kind of Agency", `[X] ${conformity.agencyKind || "Certificated Repair Station"}`, MARGIN + 5, y, font, fontBold);
  y = drawField(page, "C. Certificate Number", conformity.certificateNumber || "—", MARGIN + 5, y, font, fontBold, true);
  ...
  y = drawWrappedText(
    ...
    MARGIN + 5, y, CONTENT_WIDTH - 10, font, 7.5
  );
  y -= 5;
  page.drawText(conformity.signedBy || "—", { x: MARGIN + 10, y, size: 12, font: fontBold, color: DARK_BLUE });
  page.drawText(`Date: ${conformity.date || "—"}`, { x: MARGIN + 200, y, size: 8, font, color: BLACK });
  y -= 20;
  drawLine(page, y);

  // Section 5: Approval for Return to Service
  y = drawSectionHeader(page, "7. APPROVAL FOR RETURN TO SERVICE", y, fontBold);
  y -= 5;
  page.drawText(`[X] ${approval.status || "Approved"}    ${approval.type || "Repair Station"}    Certificate: ${approval.certificate || "—"}`, {
    x: MARGIN + 5, y, size: 8, font, color: BLACK,
  });
  y -= 15;
  page.drawText(approval.signedBy || "—", { x: MARGIN + 10, y, size: 12, font: fontBold, color: DARK_BLUE });
  page.drawText(`Date: ${approval.date || "—"}`, { x: MARGIN + 200, y, size: 8, font, color: BLACK });
  y -= 20;
  drawLine(page, y);

  // Section 6: Description of Work
  y = drawSectionHeader(page, "8. DESCRIPTION OF WORK ACCOMPLISHED", y, fontBold);
  y -= 8;
  ...
  for (const rawLine of workLines) {
    ...
    y = drawWrappedText(page, trimmed, MARGIN + 5, y, CONTENT_WIDTH - 10, font, 8);
    y -= 2;
  }
...
// Clean structured document with labeled sections.
// ══════════════════════════════════════════════════════════════
async function render8010Pdf(content: Record<string, unknown>, hash?: string | null): Promise<Uint8Array> {
  const pdf = await PDFDocument.create();
  const font = await pdf.embedFont(StandardFonts.Helvetica);
  const fontBold = await pdf.embedFont(StandardFonts.HelveticaBold);
  const fontMono = await pdf.embedFont(StandardFonts.Courier);
  ...
  // Header
  page.drawRectangle({ x: MARGIN, y: y - 30, width: CONTENT_WIDTH, height: 35, color: LIGHT_GRAY });
  page.drawText("MALFUNCTION OR DEFECT REPORT", { x: MARGIN + 10, y: y - 18, size: 14, font: fontBold, color: DARK_BLUE });
  page.drawText("FAA Form 8010-4 (10-92)", { x: MARGIN + 10, y: y - 28, size: 7, font, color: GRAY });
  page.drawText("OMB No. 2120-0663", { x: PAGE_WIDTH - MARGIN - 100, y: y - 18, size: 7, font, color: GRAY });
  y -= 50;
  drawLine(page, y);
  ...
  const componentAssembly = (content.componentAssembly || {}) as Record<string, string>;
  ...
  y = drawSectionHeader(page, "5. SPECIFIC PART CAUSING TROUBLE", y, fontBold);
  y -= 5;
  const partFields = [
    { label: "Part Name", value: defectPart.name, bold: true },
    { label: "MFG Model / Part No.", value: defectPart.partNumber, mono: true },
  ...
  for (let i = 0; i < partFields.length; i++) {
    ...
    const pFont = partFields[i].mono ? fontMono : partFields[i].bold ? fontBold : font;
    const pColor = partFields[i].mono ? DARK_BLUE : partFields[i].bold ? rgb(0.6, 0.3, 0) : BLACK;
    page.drawText(partFields[i].value || "—", { x, y: y - 11, size: 9, font: pFont, color: pColor });
  }
  y -= 30;
  drawLine(page, y);

  // Section 3: Component / Appliance
  y = drawSectionHeader(page, "6. APPLIANCE / COMPONENT (Assembly That Includes Part)", y, fontBold);
  y -= 5;
  const compFields = [
    { label: "Name", value: componentAssembly.name },
    { label: "Manufacturer", value: componentAssembly.manufacturer },
    { label: "Model / Part No.", value: componentAssembly.partNumber, mono: true },
    { label: "Serial Number", value: componentAssembly.serialNumber, mono: true },
  ];
  ...
  const metricFields = [
    ...
    { label: "Date Submitted", value: (content.dateSubmitted as string) || "—" },
  ];
  ...
  for (const rawLine of commentLines) {
    ...
    const isHeading = trimmed === trimmed.toUpperCase() && trimmed.length < 50 && trimmed.length > 3;
    if (y < 100) break;
    y = drawWrappedText(page, trimmed, MARGIN + 5, y, CONTENT_WIDTH - 10, isHeading ? fontBold : font, 8);
    y -= 2;
  }
  y -= 5;
  drawLine(page, y);
  ...
  for (let i = 0; i < subFields.length; i++) {
    ...
    page.drawText(subFields[i].value || "—", { x, y: y - 11, size: 9, font, color: BLACK });
  }
