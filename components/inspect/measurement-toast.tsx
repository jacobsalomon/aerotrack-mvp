...
interface Props {
  ...
  onReassign: (suggestion: MeasurementSuggestion) => void;
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-30 flex flex-col gap-2 w-full max-w-lg px-4">
      {visible.map((s) => (
        <div
          key={s.id}
          className="bg-zinc-800 border border-white/10 rounded-lg px-4 py-3 shadow-lg flex items-center gap-3 animate-in slide-in-from-bottom-2"
        >
          {/* Source indicator */}
          {s.source === "glasses" && (
